...
class UpdateWaybillRequest extends FormRequest
{
    ...
    {
        $waybill = $this->route('waybill');

        return $waybill instanceof Waybill
            ? ($this->user()?->can('update', $waybill) ?? false)
            : false;
    }

    public function rules(): array
    {
        /** @var Waybill|null $waybill */
        $waybill = $this->route('waybill');
        $ignoreId = $waybill instanceof Waybill ? $waybill->id : $waybill;
        ...
        return [
            ...
            'bill_no' => 'required|string|max:255|unique:waybills,bill_no,'.$ignoreId,
